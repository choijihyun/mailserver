...
public class PopClient {
	...
	public void read() throws Exception {
		...
		while (true) {
			...
			if (type < 0 || type > n) { // if client enter invalid number.
			...
			while (true) {
				...
				if (resp.toLowerCase().contains("content-type:") && resp.toLowerCase().contains("charset=")) { // parsing(case
																												// "content-type:"
																												// and
																												// "charset"
																												// is in
					...
					if (resp1.toLowerCase().contains("charset=")) {
						i = resp.indexOf(":"); // find the first index for ":" , it is in "content-type:".
						j = resp.indexOf(";"); // find the first index for ";" , it is in "content-type: ....;".
						k = resp1.indexOf("="); // find the first index for "=" , it is in "charset=".

						contentType = resp.substring(i + 1, j);
						charset = resp1.substring(k + 1);
			...
			while (true) {
				...
				if (contentTransferEncoding.contains("base64")) {
					...
					if (resp.contains("quoted-printable")) {
						flag++;
						flag1++;
						if (flag == 2 || flag1 ==2)
							resp = respBody.get(index++);
				...
				}else if(contentTransferEncoding.contains("8bit") || contentTransferEncoding.contains("7bit")) {
					if (resp.contains("8bit") || resp.contains("7bit")) {
						while (resp.equals("")) {
							resp = respBody.get(index++);
						}
						flag = 1;
